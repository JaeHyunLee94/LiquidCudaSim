...
void mpm::Engine::makeAosToSOA() {
  ...
  // Renderer reads this via .data(); an empty vector gives nullptr → SIGSEGV.
  // Initialize to 0 (default particle colour) so the pointer is always valid.
  mCurrentParticleColorWeight.assign(m_sceneParticles.size(), 0.0f);
